...
export async function createSNSTopic(
  topicName: string
): Promise<SNSTopicInterface | null> {
  const topicParams: SNS.CreateTopicInput = {
    Name: topicName,
  }
  const topicResponse: SNS.CreateTopicResponse = await sns
...
export async function createSQSQueue(
  queueName: string
): Promise<SQSQueueInterface | null> {
  const queueParams: SQS.CreateQueueRequest = {
    QueueName: queueName,
  }
  const queueResult: SQS.CreateQueueResult = await sqs
  ...
  const queueAttributesResult: SQS.GetQueueAttributesResult = await sqs
    .getQueueAttributes(queueAttributesParams)
    .promise()
...
export async function subscribeSQStoSNS({
  snsTopicArn,
  sqsQueueArn,
}: {
  snsTopicArn: string
...
export async function updateSQSPermissions({
  snsTopicArn,
  sqsQueueUrl,
  sqsQueueArn,
}: {
